...
extension Panels {
    ...
    func movePanelHeader(value: CGFloat, keyboard: Bool = false) {
        guard value > 0 else{
            return
        }
        configuration.panelVisibleArea += value
        print("height ----- \(configuration.panelVisibleArea)")
        movePanel(value: configuration.visibleArea())
//        if height <= configuration.size(for: containerView!) {
//            panelHeightConstraint?.constant += value
//        }else if height > configuration.size(for: containerView!){
//            panelHeightConstraint?.constant = configuration.size(for: containerView!)
//        }else if height < panel!.headerHeight!.constant {
////            panelHeightConstraint?.constant = panel!.headerHeight!.constant
//        }
//        delegate?.panelChanging()
//        if !keyboard {
...
//        }
    ...
    func movePanel(value: CGFloat, keyboard: Bool = false, animated: Bool = true, completion: (() -> Void)? = nil) {
        panelHeightConstraint?.constant = value
        delegate?.panelChanging()
        if !keyboard {
            panel?.headerHeight.constant += isExpanded ? -UIApplication.safeAreaBottom() : UIApplication.safeAreaBottom()
...
extension Panels {
    private func setupGestures(headerView: UIView, superview _: UIView) {
        ...
        if configuration.respondToDrag {
            let swipeUp = UISwipeGestureRecognizer(target: self, action: #selector(handleSwipGes(ges:)))
            swipeUp.direction = .up
            headerView.addGestureRecognizer(swipeUp)
            let swipeDown = UISwipeGestureRecognizer(target: self, action: #selector(handleSwipGes(ges:)))
...
extension Panels {
    ...
    @objc func handleSwipGes(ges: UISwipeGestureRecognizer) {
        if ges.state == .began {
            
        }else if ges.state == .ended {
            
        }
        let point = ges.location(in: containerView)
        print(point)
//        self.movePanel(value: point.y)
    }
