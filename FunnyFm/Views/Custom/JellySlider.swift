...
public class JellySlider: UIView {
    ...
    /// Color of track.
	public var trackColor: UIColor = UIColor.black {
        didSet {
    ...
    private func animationIntoTrackAtPosition(x: CGFloat) {
		...
		skScene.addChild(self.splashParticle(center: CGPoint(x: bubbleCenterX, y: skView.bounds.height-particleY), color: trackColor))
    }
    ...
    private let particle: SKEmitterNode = {
		let particlePath = Bundle.main.path(forResource: "SplashParticle", ofType: "sks")!
		let particle = NSKeyedUnarchiver.unarchiveObject(withFile: particlePath) as! SKEmitterNode
		let particleImage = UIImage(named: "spark.png", in: Bundle.main, compatibleWith: nil)!
	...
	override public func touchesBegan( _ touches: Set<UITouch>, with event: UIEvent?) {
        if let touch = touches.first {
			...
			DispatchQueue.main.async { [weak self] in
				...
				animation.timingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName.easeOut)
				self?.shapeLayer.add(animation, forKey: "pathAnimation")
				self?.bubbleHidden = false
				self?.touchPositionX = location.x
			}
        }
    }
