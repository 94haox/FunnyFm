...
open class AudioItem: NSObject {
    ...
    /// The artwork NSUIImage of the item.
    open var artworkImage: NSUIImage? {
        get {
            #if os(OSX)
                return artwork
            #else
    ...
    /// This can change over time which is why the property is dynamic. It enables KVO on the property.
    #if os(OSX)
    @objc open dynamic var artwork: NSUIImage?
    #else
        ...
        items.forEach {
            if let commonKey = $0.commonKey {
                ...
                case AVMetadataKey.commonKeyArtwork where artwork == nil:
                    artworkImage = ($0.value as? Data).flatMap { NSUIImage(data: $0) }
                default:
                    break
                }
            }
